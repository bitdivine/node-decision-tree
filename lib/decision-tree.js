...
function Stats() {
    ...
    function create_decision_tree(data, target_attribute, attributes, fitness_function, min_gain, max_depth) {
        ...
        var splitter = (max_depth === 0)?null: choose_attribute(data, attributes, target_attribute, fitness_function, min_gain);
        if (splitter === null) {
            ...
            Object.keys(classified_data).map(function(key){
                subtree[key] = create_decision_tree(classified_data[key], target_attribute, remaining_attributes, fitness_function, min_gain, max_depth-1);
            });
    ...
    this.create_decision_tree = function(data, target_attribute, options) {
        ...
        var max_depth = typeof(options.max_depth) === 'number'?options.max_depth:NaN;
        return create_decision_tree(data, target_attribute, attributes, fitness_function, min_gain, max_depth);
    };
